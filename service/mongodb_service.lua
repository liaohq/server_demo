...
local function newAccountColl()
	local ok, err, ret
	local c = _create_client()
	local db = c[db_name]
	
	print("newAccountColl success")
	db.account:ensureIndex({account = 1}, {unique = true, name = "account_index"})
end
...
function command.addAccount(account, passwd)
    print("addAccount account, passwd:", account, passwd)

    local ok, err, ret
    local c = _create_client()
    local db = c[db_name]

    ok, err, ret = db.account:safe_insert({account = account, passwd = passwd })
    local result = ok and ret and ret.n == 1
    if not result then
	print("addAccount error, errMsg:", err)
    	return -1
    end
...
function command.queryAccount(account, passwd)
...
skynet.start(function()
	...
	test_safe_update()
	print("mongodb test finish.", skynet.self());
